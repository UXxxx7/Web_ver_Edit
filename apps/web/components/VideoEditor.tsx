...
import {
  confirmEditJob, createEditJob, getEditJobStatus, renderEditJob, retryEditJob, reviseEditJob,
  type ActionResult,
...
export function VideoEditor() {
  ...
  // Poll while the job is in an in-progress state; stop once it lands on
  // something that's waiting on the user (or is terminal).
  useEffect(() => {
    if (!job || !IN_PROGRESS_STATUSES.includes(job.status)) return;
    const id = setInterval(async () => {
      const currentId = jobIdRef.current;
      if (!currentId) return;
      const result = await getEditJobStatus(currentId);
        ...
        <CardContent>
          ...
          >
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-medium" htmlFor="video">Video</label>
              <input
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-medium" htmlFor="edit_request">How should it be edited?</label>
              <Textarea
                id="edit_request" name="edit_request" rows={3} required
                placeholder="e.g. 剪掉空白位，加繁體字幕 / remove filler words and burn in captions"
              />
            </div>
      ...
      {job.status === "WAITING_CONFIRMATION" && job.planned_edit && (
        <Card>
          <CardContent className="flex flex-col gap-4">
            ...
            <ReviseBox
              text={reviseText} setText={setReviseText} disabled={actionPending}
      ...
      {job.status === "DONE" && (
        <Card>
          <CardContent className="flex flex-col gap-4">
            ...
            <Button variant="outline" className="w-fit" onClick={() => setJob(null)}>
              Edit another video
            </Button>
          </CardContent>
        </Card>
      ...
      {job.status === "NEEDS_CLARIFICATION" && (
        <Card>
          <CardContent>
            ...
            <Button variant="outline" className="mt-3 w-fit" onClick={() => setJob(null)}>Start over</Button>
          </CardContent>
        </Card>
      )}
    </div>
...
function JobStatusPanel({ job }: { job: EditJob }) {
