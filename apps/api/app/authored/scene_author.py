...
第 6b 条(2026-08-14 补):真实生产确认——多个真实 job 的 scene.tsx 里图形要素
的文案是硬编码字符串，一次 `props.overrides?.[...]` 读取都没有，编辑器的
recoverMissingManifestElements（客户端从源码正则恢复 manifest 的兜底机制）
因此无东西可扫，manual editor 里"Nothing to show"。根因：旧版提示词从未
声明 `overrides` 是 Props 契约的一部分，也从没要求内容字段走这条读取路径——
不是模型偷懒，是提示词本身没提过这件事。补的第 6b 条只要求内容字段
（文案/数值），不动布局/动画时序。
...
3. Read everything it needs from a single props object of THIS exact shape (already provided at render time — do not invent other props):
   type Props = {
     ...
     overrides: Record<string, Record<string, unknown>>;  // manual-editor content overrides — see rule 6b
   };
...
6b. EDITABILITY (critical — every scene you write gets opened afterward in a manual editor; a scene where nothing is editable is a defect, not a valid minimal scene): every graphic beat's actual CONTENT — headline text, a stat's value, a card's body copy, a label, anything a human would call "what it says" rather than "how it moves" — must be read from `props.overrides`, not hardcoded as a bare string/number literal. This applies EQUALLY to a fixed lookup array of phrase/lower-third text (e.g. a `const PHRASES = [{startFrame, endFrame, prefix, highlight, suffix}, ...]`-style table driving on-screen captions or lower-thirds) — a confirmed real defect, 2026-08-14: a scene wired its main graphic cards through overrides correctly but left an entire separate array of lower-third phrase text completely hardcoded, because it wasn't obviously "a card." Every entry's text fields need the SAME treatment as any other beat's content — this rule covers ANY on-screen text a human would want to edit, not just things that visually look like a distinct card. For each such element (a card, OR one entry in a phrase table): invent a short stable id (e.g. `"title-card"`, `"quote-1"`, `"stat-signups"`, `"phrase-2-prefix"`) and read every one of its editable fields as
   `(props.overrides?.["<id>"]?.<field> as string) ?? "<the text you'd otherwise have hardcoded>"`
   — `as number` for a numeric field, same `?? <original value>` fallback shape either way. This is a CONTENT mechanism only: never wire position/size/stacking (x/y/w/h/layer) or spring/interpolate EASING this way, only the words/numbers a user would actually want to correct or reword later. Prefer these field names when they fit, so recovered elements classify sensibly in the editor: `headline`+`value` for a stat-style card, `text` for a plain text block, `src`+`label` for an image/b-roll-style element. Every distinct graphic beat needs at least one such read — a scene with graphic beats but zero `props.overrides?.[...]` reads anywhere is exactly the defect this rule exists to prevent.
   ALSO wire each beat's own on-screen WINDOW — the frame it mounts and the frame it's gone — the exact same way, using the field names `mountFrame`/`endFrame` (these two exact names, nothing else): `const _mount = (props.overrides?.["<id>"]?.mountFrame as number) ?? <the frame you'd otherwise have hardcoded as its entrance trigger>;` and the same shape for `endFrame` (omit `endFrame` entirely, matching whatever you'd naturally write, only for a beat that's genuinely meant to stay up for the rest of the video). Use `_mount`/an equivalent local, not `props.sourceFrame` directly, everywhere you currently gate that beat's entrance/exit. Without this, the editor has no way to know WHEN each beat actually appears — every beat defaults to "visible for the entire video," which is wrong for anything but a persistent element like a caption, and makes distinct beats impossible to tell apart on the timeline.
...
10. NEVER let bare canvas show through as plain white — Remotion's default composition background is flat white, and it reads as a broken/blank render anywhere it's visible: the letterboxing bars around a contained (non-cropped) landscape b-roll, a full-canvas graphic beat with no b-roll or person video under it, or any other area you fill with a plain <div>/<AbsoluteFill> rather than real content. Every such fill MUST use the brand's grid-paper background, not a solid color — set it as the `background` (not `backgroundColor`) CSS shorthand, exactly:
   `"linear-gradient(rgba(20,20,24,0.09) 1px, transparent 1px) 0 0 / 88px 88px, linear-gradient(90deg, rgba(20,20,24,0.09) 1px, transparent 1px) 0 0 / 88px 88px, #FAFAF8"`
   (a thin gray ruled grid over a near-white paper tone — this is the brand identity, matching every other scene in this product, not a placeholder to swap out).
...
3. The Props contract is UNCHANGED and fixed: `videoSrc`, `sourceFrame`, `broll`, `words`, `fps`, `durationInFrames`, `width`, `height`, `overrides`. `export const AuthoredScene: React.FC<Props>` + `export default AuthoredScene;`. Import only from "react" and "remotion". Self-contained, deterministic from `props.sourceFrame` — NOT `useCurrentFrame()` for content timing (sourceFrame is what stays correct once a viewer makes a mid-video cut; useCurrentFrame() doesn't). B-ROLL PLAYBACK TIMING: every b-roll <OffthreadVideo> MUST be wrapped in <Sequence from={startFrame} durationInFrames={endFrame - startFrame}> so the clip plays from its own start, never the absolute composition frame (a bare b-roll OffthreadVideo freezes on its last frame once the window starts past the clip length). EDITABILITY: if the code you're revising already reads a graphic beat's content from `props.overrides?.["<id>"]?.<field> ?? <default>`, keep that EXACT id and field name — the editor's UI and any of the user's already-saved edits key on it, and renaming it silently orphans them. If a graphic beat you're touching has NO such read at all (a hardcoded string/number where the content should be user-editable), add one in the same `(props.overrides?.["<id>"]?.<field> as string) ?? "<original text>"` shape rather than leaving it hardcoded — but only for beats the current defects/notes already have you touching; do not go wire up unrelated, untouched parts of the scene as a side effect of this pass. The same applies to a beat's on-screen window — `mountFrame`/`endFrame` read from overrides the exact same way (these two exact field names) — keep them if already wired, add them if a beat you're touching hardcodes its entrance/exit frame instead.
4. Landscape b-roll never side-cropped (contain + blurred fill). The base person video is rendered by a wrapper OUTSIDE your scene, already full-screen behind it — your scene must NEVER render its own `<OffthreadVideo>`/`<Video>` with `src={videoSrc}` (or `props.videoSrc}`). IF THE RENDER YOU'RE FIXING PRODUCED ZERO FRAMES / CRASHED IMMEDIATELY (not a visual defect, a hard failure): check for exactly this FIRST — a leftover base-video element is the single most common cause, because it makes two elements request the same video source at once and reliably crashes the real server render. Remove it entirely; do not just swap which video component renders it. A frame showing plain white canvas (letterboxing bars around contained b-roll, or a full-canvas graphic beat with nothing under it) is ALSO a confirmed defect — replace that fill's `background` with the brand grid: `"linear-gradient(rgba(20,20,24,0.09) 1px, transparent 1px) 0 0 / 88px 88px, linear-gradient(90deg, rgba(20,20,24,0.09) 1px, transparent 1px) 0 0 / 88px 88px, #FAFAF8"`, never a solid color. Captions stay in sync from `words`. Preserve the timing, windows, and transitions of EXISTING b-roll inserts unless a flagged defect or the user's notes require changing them — do NOT restyle, re-time, or flatten working b-roll as a side effect of another fix.
